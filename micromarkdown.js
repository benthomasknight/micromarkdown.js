//
// resultHtml = micromarkdown.parse(sourceText);
//
var micromarkdown = {
    useajax: false,
    regexobject: {
        ...
        hr: /^(?:([\*\-_] ?)+)\1\1$/gm,
        lists: /^((\s*((\*|\-)|\d+(\.|\))) [^\n]+)\n)+/gm,
        noteInList: /^([ \t]*)((\d).*[\s]+)(\*\*note:\*\*.*)(\s*)(\d)/im, // Hacked in to find notes wetween bulleted lists
        bolditalic: /(?:([\*_~]{1,3}))([^\*_~\n]+[^\*_~\s])\1/g,
        links: /!?\[([^\]]+)\] *\(([^\)<>]+)( "[^\(\)\"]+")?\)/g,
        urlInLinks: /(?:!?\[(?:[^\]]+)\] *\()([^\)<>]+)( "[^\(\)\"]+")?\)/, // we need to remove the url from a few sections so it doesnt get rendered as markdown
        reflinks: /\[([^\]]+)\]\[([^\]]+)\]/g,
    ...
    parse: function (str, strict) {
        ...
        /* lists */
        var skipped = [];
        while ((stra = micromarkdown.regexobject.noteInList.exec(str)) !== null) {
            if (Number(stra[3]) + 1 == Number(stra[6])) {
                str = str.replace(stra[4], stra[1] + stra[1] + "* " + stra[4]);
            } else {
                // Remove notes that can be incorrectly analyzed as in the middle of a list
                str = str.replace(stra[4], "<escaping>" + stra[4] + "</escaping>\n" + stra[5]);
                skipped.push(stra[4]);
            }
        }
        while ((stra = micromarkdown.regexobject.lists.exec(str)) !== null) {
            var initialIndent = null;
            casca = 0;
            ...
            for (i = 0; i < helper.length; i++) {
                if ((line = /^((\s*)((\*|\-)|\d+(\.|\))) ([^\n]+))/.exec(helper[i])) !== null) {
                    if (initialIndent == null) {
                        initialIndent = line[2].length;
                    }
                    if ((line[2] === undefined) || (line[2].length <= initialIndent)) {
                        nstatus = 0;
                    } else {
                        if (indent === false) {
        ...
        // Replace all notes that are between lists but not concurrent lists
        for (var i = 0; i < skipped.length; i++) {
            str = str.replace("<escaped>" + skipped[i] + "</escaped>", skipped[i]);
        }
        ...
        /* for bold and italic, the syntax may match within a url. We dont want it to convert parts of urls so they are taken out for this bit*/
        var pageSplit = [];
        while ((stra = micromarkdown.regexobject.urlInLinks.exec(str)) !== null) {
            /*add an array to pop and push links to they arent rendered as markdown*/
            var key = '<!--link-placeholder-' + pageSplit.length + '-->';
            str = str.replace(stra[1], key);
            pageSplit.push([key, stra[1]]);
        }
        ...
        /* now that the rendering has been done, replace the links*/
        for (var i = 0; i < pageSplit.length; i++) {
            str = str.replace(pageSplit[i][0], pageSplit[i][1]);
        }
    ...
    while ((stra = micromarkdown.regexobject.links.exec(str)) !== null) {
      if (stra[0].substr(0, 1) === '!') {
        ...
        str = str.replace(stra[0], '<a ' + micromarkdown.mmdCSSclass(stra[2], strict) + 'href="' + stra[2] + '" target="_blank">' + stra[1] + '</a>');
      }
    }
    while ((stra = micromarkdown.regexobject.mail.exec(str)) !== null) {
      str = str.replace(stra[0], '<a href="mailto:' + stra[1] + '" target="_blank">' + stra[1] + '</a>');
    }
    while ((stra = micromarkdown.regexobject.url.exec(str)) !== null) {
      ...
      str = str.replace(stra[0], '<a ' + micromarkdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '" target="_blank">' + repstr.replace(/(https:\/\/|http:\/\/|mailto:|ftp:\/\/)/gmi, '') + '</a>');
    }
    while ((stra = micromarkdown.regexobject.reflinks.exec(str)) !== null) {
      helper1 = new RegExp('\\[' + stra[2] + '\\]: ?([^ \n]+)', "gi");
      if ((helper = helper1.exec(str)) !== null) {
        str = str.replace(stra[0], '<a ' + micromarkdown.mmdCSSclass(helper[1], strict) + 'href="' + helper[1] + '" target="_blank">' + stra[1] + '</a>').replace(/^\s+|\s+$/g, '');
    ...
    while ((stra = micromarkdown.regexobject.smlinks.exec(str)) !== null) {
      ...
      str = str.replace(stra[0], '<a ' + micromarkdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '" target="_blank">' + stra[1] + '</a>');
    }
    while ((stra = micromarkdown.regexobject.url2.exec(str)) !== null) {
      repstr = stra[1];
      str = str.replace(stra[0], '<a ' + micromarkdown.mmdCSSclass(repstr, strict) + 'href="' + repstr + '" target="_blank">' + repstr + '</a>');
    }
